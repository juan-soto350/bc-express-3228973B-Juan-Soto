...
import { Machine, PaginatedResponse, SingleResponse, ValidationErrorResponse } from '../types';
...
export async function getAll(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    res.json(result satisfies PaginatedResponse<Machine>);
  } catch (err) {
    next(err);
  }
}
...
export async function getById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const machine = await service.findById(parsed.data);
    res.json({ data: machine } satisfies SingleResponse<Machine>);
  } catch (err) {
    next(err);
  }
}
...
export async function create(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    res.status(201).json({ data: machine } satisfies SingleResponse<Machine>);
  } catch (err) {
    next(err);
  }
}
